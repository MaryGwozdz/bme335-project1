...
for x = 1:length(a)
    for y = 1: length(b)
        ...
        totT1 = totT1 + pdf_1(y,x);
        totT2 = totT2 + pdf_2(y,x);
    end
end

%Create Marginals
for x = 1:length(a)
    mPDFaT1(x) = sum(pdf_1(:,x))/totT1; %Marginal PDF for A in T1 cells
    ...
    mPDFaT2(x) = sum(pdf_2(:,x))/totT2; %Marginal PDF for A in T2 cells
...
for y = 1:length(b)
    mPDFbT1(y) = sum(pdf_1(y,:))/totT1; %Marginal PDF for B in T1 cells
    ...
    mPDFbT2(y) = sum(pdf_2(y,:))/totT2; %Marginal PDF for B in T2 cells
